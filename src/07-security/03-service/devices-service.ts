...
class DevicesService {
    ...
    async findSessionToTerminate(deviceId: string): Promise<string | null> {
        return await devicesMongoRepository.findSessionByDeviceId(deviceId)

    }

    async isUserCanTerminateSession(refreshToken: string, deviceId: string): Promise<boolean> {
        const userId = await jwtService.getUserIdByToken(refreshToken)
        const isSessionBelongsToUser = await devicesMongoRepository.findSessionByDeviceId(deviceId)
